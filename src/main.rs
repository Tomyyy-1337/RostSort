    ...
    umstz Sortierung{
            ...
            T: Kopieren + Hinein<u32>,
        {
            ...
            für &wert in scheibe.wieder() {
                lass index = wert.hinein() als usize;
                eimer[index] += 1;        
            }

            lass änd j = 0;
            für (i, anzahl) in eimer.wieder().indizieren().filter(|(_, anzahl)| **anzahl > 0) {
                lass wert: T = gefährlich { std::ptr::read(&i als *konstante usize als *konstante T)};
